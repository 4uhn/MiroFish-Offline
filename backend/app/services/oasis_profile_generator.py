...
Improvements:
1. Leverages graph retrieval to enrich node information
...
class OasisProfileGenerator:
    ...
    Converts entities from the knowledge graph into Agent Profiles for OASIS simulation.

    Key features:
    1. Leverages graph retrieval for richer context
    ...
    def _search_graph_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use GraphStorage hybrid search to retrieve rich information about the entity.
        ...
        try:
            ...
            logger.info(f"Graph hybrid search completed: {entity_name}, retrieved {len(results['facts'])} facts, {len(results['node_summaries'])} related nodes")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Rich information from graph hybrid search
        """
        context_parts = []
        ...
        # 4. Use graph hybrid search for richer information
    ...
    def _generate_profile_with_llm(
        ...
        # Retry generation until successful or max attempts reached
        max_attempts = 3
        last_error = None
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are a social media persona generation expert. Generate detailed, realistic personas for social media simulation, faithfully reproducing known real-world circumstances as much as possible. You MUST return valid JSON format, and all string values must not contain unescaped newline characters. You MUST respond in English only."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, faithfully reproducing known real-world circumstances as much as possible.
...
1. bio: Social media bio, 1-2 sentences (max 50 words)
2. persona: Persona description (500-800 words of plain text), must include:
   - Basic information (age, occupation, location)
   - Background (connection to events, key relationships)
   - Personality traits (MBTI type, communication style)
   - Social media behavior (posting style, language characteristics)
   - Stances and views (attitudes toward the key topics)
   - Personal memory (this individual's connection to events and their reactions)
3. age: Age as a number (must be a realistic integer — vary ages, do NOT default to 30)
4. gender: Gender, must be in English: "male" or "female"
5. mbti: MBTI type (vary across agents — do NOT always use ISTJ, use a diverse mix)
...
- Respond entirely in English (gender must be "male" or "female")
- Content must be consistent with the entity information
- age must be a valid integer, gender must be "male" or "female"
- Do NOT fabricate organizations, events, or backstories not supported by the context
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an institutional/group entity, faithfully reproducing known real-world circumstances as much as possible.
...
1. bio: Official account bio, 1-2 sentences (max 50 words), professional and appropriate
2. persona: Account profile description (500-800 words of plain text), must include:
   - Basic institutional information (official name, nature, primary functions)
   - Account positioning (target audience, core communication functions)
   - Communication style (tone, language characteristics)
   - Stances and attitudes (official positions on core topics)
   - Institutional memory (this institution's connection to events and its responses)
...
5. mbti: MBTI type, used to describe account style (vary across institutions — ISTJ, ESTJ, INFJ, etc.)
6. country: Country (e.g., "United States", "China")
...
- Respond entirely in English (gender must be "other")
