...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics - match "相关预测事实: X条" or "Related Prediction Facts: X" format
    const factMatch = text.match(/(?:相关预测事实|Related Prediction Facts):\s*(\d+)/)
    const entityMatch = text.match(/(?:涉及实体|Entities Involved):\s*(\d+)/)
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/(?:当前有效事实|Currently Active Facts):\s*(\d+)/)
    ...
    const activeSection = text.match(/### (?:【当前有效事实】|\[Currently Active Facts\])[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    const entitySection = text.match(/### (?:【涉及实体】|\[Entities Involved\])\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const reasonMatch = text.match(/### (?:采访对象选择理由|Interview Subject Selection Rationale)\n([\s\S]*?)(?=\n---\n|\n### )/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*(?:选择|Selected|Chose)\s*([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
    ...
    const summaryMatch = text.match(/### (?:采访摘要与核心观点|Interview Summary and Core Insights)\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/(?:找到\s*(\d+)\s*条|Found\s*(\d+)\s*results)/)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（该平台未获得回复）' || t === '(该平台未获得回复)' || t === '[无回复]' || t === '(No response received on this platform)'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Prefer to try "问题X：" or "Question X:" or "Q X:" format
      const cnPattern = /(?:^|[\r\n]+)(?:问题|Question|Q)\s*(\d+)[：:]\s*/g
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after "最终答案:" or "Final Answer:"
